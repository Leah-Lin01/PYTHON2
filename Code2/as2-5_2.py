...
def scan_image_with_vlm(image):
    ...
    try:
        ...
        6. 分壓電路（Voltage Divider）判斷：請特別注意「兩顆電阻上下串聯」的拓樸結構——
           上方電阻一端接電源軌（如 +V3.3SB_WWAN），另一端接到一個有網路名稱標籤（net label）
           的節點（例如 WWAN_PCIE_USB_SW）；下方電阻的一端接到同一個節點，另一端接地(GND/接地符號)。
           也就是「電源 → 上電阻 → 網路節點 → 下電阻 → GND」的結構，就是電壓分壓電路。
           - 找到這種結構時，將上方電阻標記 "div_role": "TOP"、下方電阻標記 "div_role": "BOT"，
             同一組分壓請給相同的 "div_group"（直接用中間那個網路節點的名稱字串即可，
             例如兩顆都填 "WWAN_PCIE_USB_SW"），不同組分壓要給不同的 div_group。
           - 不是分壓電路的一般電阻（只有單邊接電壓軌、另一邊直接接地或接晶片腳位），
             "div_group" 與 "div_role" 請填空字串 ""。
        ...
        JSON 格式範例（第一組為分壓電路，第二個為一般電阻）：
        [
          {"name": "R156", "val": "10K", "p": "1/16W", "v": "3.3V", "div_group": "WWAN_PCIE_USB_SW", "div_role": "TOP"},
          {"name": "R163", "val": "12K", "p": "1/16W", "v": "3.3V", "div_group": "WWAN_PCIE_USB_SW", "div_role": "BOT"},
          {"name": "PR330", "val": "1K", "p": "1/16W", "v": "1.8V", "div_group": "", "div_role": ""}
        ]
        """
        ...
        for item in parsed_data:
            ...
            r_div_group = (item.get("div_group") or "").upper().strip()
            r_div_role = (item.get("div_role") or "").upper().strip()
